...
class GMM():
    ...
    def _e_step(self, features):
        ...
        """
        post_array = np.empty([features.shape[0], self.n_clusters])
        for n in range(0, self.n_clusters):
            post_array[:,n] = self._posterior(features,n)
    ...
    def _m_step(self, features, assignments):
        ...
        T_no_left_hand = np.sum(assignments,axis=0)

 
        mu = np.empty([self.n_clusters,np.shape(features)[1]])
        #update mean
        for j in range(0, len(mu)):
            xs = np.empty(np.shape(features))
            for i in range(0, len(features)):
                xs[i] = features[i] * assignments[i,j]
            xs = np.sum(xs,axis=0)
            mu[j] = xs/T_no_left_hand[j]
        new_means = mu

        #update weights
        w = T_no_left_hand/len(features)

        #update matrix
        cov_matrix = np.empty([self.n_clusters, np.shape(features)[1]])
        for j in range(0,len(cov_matrix)):
            xs = np.empty(np.shape(features))
            for i in range(0, len(features)):
                xs[i] = (features[i] - mu[j])
                xs[i] = xs[i] ** 2 
                xs[i] = xs[i] * assignments[i,j]
            xs = np.sum(xs, axis=0)
            cov_matrix[j] = xs/T_no_left_hand[j]
        cov_matrix = np.mean(cov_matrix, axis=1)


        return mu, cov_matrix, w
        raise NotImplementedError()
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        """
        raw_likelihood = multivariate_normal.logpdf(features, mean=self.means[k_idx], cov=self.covariances[k_idx])
        incorp_weights = np.log(self.mixing_weights[k_idx]) + raw_likelihood
        return incorp_weights
